...
def check_transactions_route():
    try:
        ...
        response.headers.add('Content-Type', 'application/json')  # Explicitly set content type
        return response
    except Exception as e:
        print(f"Error in check_transactions_route: {str(e)}")
        error_response = {"error": str(e)}
        response = jsonify(error_response)
        response.headers.add('Content-Type', 'application/json')  # Also for error responses
        return response, 500
